...
def load_data(dir_path, dataset_type, n_sub, mode='train'):
    ...
    if mode == 'train':
        ...
        mode_s = 'E'
    if dataset_type == 'HGD':
        data_mat = scipy.io.loadmat('E:\\BaiduNetdiskDownload\\hgd\\' + '{:1d}.mat'.format(n_sub))
        if mode_s == 'T':
            data = data_mat['traindata']
            label = data_mat['trainlabel'] + 1
        else:
            data = data_mat['testdata']
            label = data_mat['testlabel'] + 1
    elif dataset_type == 'O':
        data_mat = scipy.io.loadmat('/root/' + '{}{:02d}{}.mat'.format(dataset_type, n_sub, mode_s))

        # 加载数据并转置
        if mode_s == 'T':
            data = data_mat['data']
            data = np.transpose(data, (1, 2, 0))  # 假设维度为(通道, 时间点, 试验)
        ...
        # # 滤波器参数
        # fs = 250
        # b, a = butter(4, [4 / (fs / 2), 40 / (fs / 2)], btype='band')
        #
        # # 合并试验和通道维度，直接向量化滤波
        # data_filtered = filtfilt(b, a, data.reshape(-1, 1000).astype(np.float64), axis=1)
        # data = data_filtered.reshape(data.shape)
        if dataset_type == 'C':
            data = data[:, :, 125:1125]
        print(data.shape)
        # if dataset_type == 'B':
...
def numberClassChannel(database_type):
    if database_type == 'A':
        ...
        number_class = 3
        number_channel = 26
    elif database_type == 'HGD':
        number_class = 4
        number_channel = 45
    elif database_type == 'O':
        number_class = 2
        number_channel = 62
...
class CustomDatasets(Dataset):
    ...
    def __init__(self, seq_data, fre_data, event_data):
        ...
        参数:
        seq_data: 顺序数据，用于模型输入。
        fre_data: 频域数据，用于模型输入。
        event_data: 事件标签，用于模型训练或评估。
        """
    ...
    def __getitem__(self, index):
        ...
        包含顺序数据、频域数据和标签的元组。
        """
        # 将顺序数据转换为 PyTorch Tensor
        seq_data = torch.Tensor(self.seq_data[index])
        # 将频域数据转换为 PyTorch Tensor
        fre_data = torch.Tensor(self.fre_data[index])
        # 将标签转换为 PyTorch Tensor，注意：类型应与损失函数要求一致，此处改为 torch.Tensor 以提高灵活性
